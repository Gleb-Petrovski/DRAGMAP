...
const char* ValidatorThread::validateOne(std::vector<std::uint8_t>& block, const char* pCigarEnd){
...
void ValidatorThread::validateBlock(std::vector<std::uint8_t>& block){
  ...
  assert(std::distance(reinterpret_cast<const char*>(&block.front()), pCigarEnd) == block.size());
}
